...
const CreateRefund = ({navigation}: any) => {
  ...
  const handleSubmit = async () => {
    if (!selectedPatient) return Alert.alert('Validation', 'Please select patient');
    if (refundType === 'Select Refund Type') return Alert.alert('Validation', 'Please select refund type');
    if (billType === 'Select Bill Type') return Alert.alert('Validation', 'Please select bill type');
    if (!refundAmount || isNaN(Number(refundAmount)) || Number(refundAmount) <= 0) return Alert.alert('Validation', 'Enter valid amount');
    if (!refundDate) return Alert.alert('Validation', 'Please select refund date');
  ...
  .split('T')[0],
      ...
      Alert.alert('Success', 'Refund submitted', [{text: 'OK', onPress: () => navigation.goBack()}]);
    } catch (error:any) {
      console.log(
  'Create refund error',
  error?.response?.data,
);
      Alert.alert('Error', 'Failed to create refund');
    }
  };
  ...
  return (
    <ScrollView style={[styles.container, {backgroundColor: colors.background}]} contentContainerStyle={{paddingBottom: sizes.padding * 2}}>
      <Text h4 semibold style={{marginBottom: 18}}>Create Refund</Text>
      ...
      <View style={[styles.card, {backgroundColor: colors.card}] }>
        ...
        <View style={styles.actionRow}>
          ...
          <TouchableOpacity style={[styles.actionButton, styles.submitButton, {backgroundColor: PRIMARY_PINK}]} onPress={handleSubmit}>
            <Text white bold>SUBMIT REFUND</Text>
          </TouchableOpacity>
        </View>
      </View>
    </ScrollView>
