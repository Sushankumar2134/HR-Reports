...
import {getPatientAppointments} from '../../api/patientPortalApi';
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View style={styles.tableMinWidth}>
            <View style={[styles.row, styles.headerRow]}>
              ...
              <TableCell title="PATIENT" width={220} header />
              <TableCell title="DATE" width={160} header />
              <TableCell title="TIME" width={140} header />
            ...
            {rows.length ? (
              rows.map((item, index) => (
                <View key={item.id} style={styles.row}>
                  ...
                  <TableCell title={item.patientName} width={220} />
                  <TableCell title={item.date} width={160} />
                  <TableCell title={item.time} width={140} />
                  <TableCell title={item.status} width={140} />
                </View>
              ))
            ) : (
              <View style={[styles.emptyState, styles.tableMinWidth]}>
                <Text color="#64748b">No appointments available</Text>
              </View>
            )}
          </View>
        </ScrollView>
